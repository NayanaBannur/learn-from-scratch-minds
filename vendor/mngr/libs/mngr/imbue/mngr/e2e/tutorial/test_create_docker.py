...
def test_create_docker_default_image(e2e: E2eSession) -> None:
    ...
    # command exited 0. Scope the listing with `--provider docker` rather than
    # filtering the output with `--include`: `mngr list` defaults to
    # --on-error abort and discovers across every enabled provider, so an
    # enabled-but-unreachable one (here, the Modal backend the monorepo venv
    # installs but this job holds no token for) aborts the whole listing with
    # exit 6 before any agent is reported. `--include` cannot prevent that --
    # it only filters what discovery already returned. Same reasoning and
    # pattern as test_create_commands.py's `mngr list --provider local`.
...
def test_create_docker_custom_dockerfile(e2e: E2eSession) -> None:
    """Tutorial block:
        # use a custom Dockerfile for the container image. `-b` values are passed to `docker build` verbatim, so the
        # Dockerfile is selected with docker's own `--file`. One strange thing is that you probably want to pass "-b ."
        # because that's just how docker works (it takes the context dir as the last arg)
